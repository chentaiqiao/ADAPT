...
from random_walk_comm.utils.util import get_gard_norm, huber_loss, mse_loss
from random_walk_comm.utils.valuenorm import ValueNorm
from random_walk_comm.algorithms.utils.util import check
from random_walk_comm.algorithms.utils.transformer_act import *
...
class MATTrainer:
    ...
    def train_priority_scoring_network(self,dependency_vectors, final_P,n_agents, num_epochs, learning_rate):
        ...
        print(f"dependency_vectors.shape", dependency_vectors.shape)
        print(f"dependency_vectors", dependency_vectors)
        ...
        print(f"final_P_tensor.shape", final_P_tensor.shape)
        print(f"final_P_tensor", final_P_tensor)
        ...
        print(f"Model device: {next(scoring_network.parameters()).device}")
        print(f"Input device: {dependency_vectors.device}")
    
        for name, param in scoring_network.named_parameters():
        ...
        for epoch in range(num_epochs):
            ...
            #     raise ValueError("Logits do not require grad, check network and inputs")
    ...
    def ppo_update(self, sample, steps, index, total_step=0):
        ...
        for index in range(len(dependency_vectors_list)):
            self.train_priority_scoring_network(
                dependency_vectors_list[index],
                final_P_list[index],
                self.num_agents, 
                num_epochs=self.ppo_epoch,
