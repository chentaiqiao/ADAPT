...
def discrete_autoregreesive_act(args, decoder, obs_rep, obs, relation_embed, relations, batch_size, n_agent, action_dim, tpdv,
    ...
    def local_optimization(P, dependency_vectors, Threshold_loc_radio):
        ...
        for i in range(n_agent):
            for j in range(n_agent):
                ...
                if len(dependency_history) > 1000:
    ...
    if len(difference_history) > 1000:
        difference_history.pop(0) 
    last_W=W  
    if (wi_diff_norms > Threshold_Real).any():#any agent need adjust ：run B1+B2
        ...
        global dependency_vectors_list
        global final_P_list
        if len(final_P_list) > 10:
